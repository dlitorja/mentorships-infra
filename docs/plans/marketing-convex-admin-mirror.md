...
**Status (2026-09-24):** 7-PR arc **complete**. PRs #858 (PR 5), #859 (PR 6a+6b), #866 (PR 6), #868 (PR 7), and #861 (Turnstile follow-up that also closed apps/platform parity) all merged to `main`. The schema changes from PR 7 (new indexes on `marketingWaitlist` + new `inventoryChangeLog` table) are pending manual Convex prod deploy per §5.3 — tracked as **HUC-40** (PR-7 verification). Three open follow-ups remain (see "What remains after PR 7" at the bottom of §4f): Cleanup PR (delete Supabase `apps/marketing/app/api/waitlist/route.ts`), Kajabi webhook writer migration (**HUC-41**), and `packages/email-templates` workspace package to de-duplicate `buildWeeklyDigestEmail`. None of the three have a clear owner or target date, so they are tracked in this plan rather than in Linear per AGENTS.md.
**Target base:** `main`  
...
PR #859 added 1 new action (`waitlist.actionAddToWaitlist`) and 2 env vars to `convex/convex.config.ts` but did NOT modify `convex/schema.ts`. No new schema-change verification issue required. Existing tracking issue: **HUC-38** (state `Backlog` → `In Progress` after merge).
...
### Status: ✅ Merged (2026-09-21) as part of PR #859 (6a+6b combined)

First implementation landed one commit (`ae4e1001`) on `feat/marketing-port-addtowaitlist`. Greptile held at **0/5** with four P1 comments that expose structural gaps the original spec did not anticipate. Branch reset to `main`, force-pushed, deleted from origin. The reset implementation reached Greptile **4/5 on commit `f4a2d98c`** after 5 rounds of fixes; squash-merged as PR #859 on 2026-09-21T05:46:20Z. All four P1s resolved. Convex prod deploy succeeded against `prod:fine-bulldog-260`. Schema unchanged (`marketingWaitlist.by_email_and_instructorSlug_mentorshipType` index already existed from PR #854).
...
**No — schema unchanged in PR #859.** The `by_email_instructorSlug_mentorshipType` compound index was already present in `convex/schema.ts` from PR #854 (Convex foundation). PR #859 only added a new public action and 2 env vars to `convex/convex.config.ts`. Manual Convex prod deploy still required to push the new code: `CONVEX_DEPLOYMENT=prod:fine-bulldog-260 npx convex@1.45.0 deploy` (executed 2026-09-21).
...
Tracking issue: **HUC-38** (state `In Progress` after merge, target `Done` once post-merge smoke tests complete). Smoke tests:
1. `mentorships.huckleberry.art/instructors/<slug>` opens without a build error in the marketing CI build log (`pnpm build`). The original P1 #2 surfaced as a static prerender crash — confirming it builds clean is the first smoke test. *Pre-existing build issues on `/instructors/<slug>/courses` and `/api/free-mentorship` are unrelated to PR #859 — Supabase env vars / SSR init.*
...
9. `CONVEX_DEPLOYMENT=prod:fine-bulldog-260 npx convex@1.45.0 deploy` succeeds — schema change verified deployed to prod. *Done (2026-09-21).*
...
Single source of truth for the waitlist. Status as of 2026-09-21:
...
1. **Kajabi webhook still writes to Supabase `inventory_change_log`.** New inventory changes won't appear in `inventoryChangeLog` Convex until a follow-up webhook migration. PR 7 backfills the existing rows via `scripts/migrate-inventory-change-log.ts` so the digest email is populated for historical data; going forward, the section will be empty until the webhook follow-up. Tracked as **HUC-41** (writer migration in the Kajabi webhook + admin manual-update routes).
...
The 7-PR arc is complete. Three outstanding follow-ups (no clear owner / target date, so tracked in the plan rather than in Linear per AGENTS.md "deferred-but-tracked work" rule):

- **Cleanup PR**: delete `apps/marketing/app/api/waitlist/route.ts` (Supabase POST handler) — last remaining public Supabase write path. Per §4e limitation #2. Small PR — search for any remaining callers (currently the marketing route is reachable but no UI points at it after PR #859 + PR #861).
- **Kajabi webhook Convex migration**: replace the Supabase-backed `logInventoryChange` in `apps/marketing/app/api/webhooks/kajabi/route.ts` with a Convex write. Per PR 7 limitation #1. Tracked: **HUC-41** (writer migration scope).
- **`packages/email-templates` workspace package** to de-duplicate `buildWeeklyDigestEmail` between Convex action + marketing route. Per PR 7 limitation #2. ~50 lines of duplicated code today.

...
- **TOCTOU gap in `/api/instructor/session-packs/[sessionPackId]/route.ts:142-147`.** Concurrent soft-delete between the upfront `deletedAt` check and the `setRemainingSessions` / `restoreSessionCounts` mutation returns `updatedPack === null`, which falls through to a generic HTTP 500 "Failed to update session pack" instead of a 404. The caller can't distinguish "deleted" from "server error". **Resolution:** add a 404 branch when `updatedPack === null` so the client gets an actionable signal.

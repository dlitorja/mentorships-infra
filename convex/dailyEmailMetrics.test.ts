...
test("parseMetricsResponse: counts rows whose period is not YYYY-MM-DD as unparseable", () => {
  const ingestedAt = 1_700_000_000_000;
  const payload = {
    data: [
...
test("parseMetricsResponse: empty data array yields no rows and no unparseable", () => {
  expect(parseMetricsResponse({}, 1_700_000_000_000)).toEqual({ rows: [], unparseableRows: 0 });
  expect(parseMetricsResponse({ data: [] }, 1_700_000_000_000)).toEqual({ rows: [], unparseableRows: 0 });
});

test("parseMetricsResponse: all rows unparseable yields unparseableRows=N and rows=[]", () => {
  const { rows, unparseableRows } = parseMetricsResponse(
    { data: [{ period: "bad" }, { period: "worse" }] },
    1_700_000_000_000
  );
  expect(rows).toEqual([]);
  expect(unparseableRows).toBe(2);
});
...
test("upsertDailyMetrics: same count and source with later ingestedAt does not patch (avoids write churn)", async () => {
  const t = convexTest(schema, modules);
...
test("fetchAndStore: response with all unparseable rows throws to surface cron failure", async () => {
  const fetchMock = globalThis.fetch as unknown as ReturnType<typeof vi.fn>;
  fetchMock.mockResolvedValueOnce(
    jsonResponse({
      data: [{ period: "garbage1" }, { period: "garbage2" }],
    })
  );
  ...
  ).rejects.toThrow(/unparseable rows/);

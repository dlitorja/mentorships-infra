        ...
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Sign up to potentially be selected for a free one-on-one mentorship session{instructorSlug ? (
              <> with <span className="font-semibold">{instructorSlug.replace(/-/g, " ")}</span></>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <Button asChild variant="outline" className="w-full">
              <Link href={instructorSlug ? `/instructors/${instructorSlug}` : "/instructors"}>Back to Instructor</Link>
            </Button>
          </div>
        </CardContent>
      </Card>

...
Last Updated: June 3, 2026 (Booking Flow Consolidation: calendar slot picker (month view) + removed legacy BookSessionForm; Session Notifications COMPLETED: cancel/reschedule/30-min reminder emails via Trigger.dev; Instructor Dashboard Enhanced: past sessions, session actions (reschedule/cancel/notes), student deep dive; Docstrings on session email templates (#390))
...
- `availableDays` off-by-one: last day of every month was always disabled (fixed by deriving day numbers directly from `slotsByDay` keys instead of date range filtering)
- Stale `selectedSlot` after month navigation: parent now clears selection via `onNavigate` callback
- Redundant `isPast` ternary: removed dead branch (both branches identical)

**Consolidation**:
- Removed legacy `BookSessionForm` from `/calendar` page — `BookWithGoogle` is now the only booking path
- `BookSessionForm` created sessions without Google Calendar events, no reminder emails, no Discord link in events
- Deleted `apps/platform/components/calendar/book-session-form.tsx`

**Not Implemented** (per preference):
- Expose calendar config in instructor dashboard (one-time setup in Settings is adequate)

**Status**: ✅ COMPLETED - June 3, 2026 (PR #391)
...
**PRs**: Booking Flow Consolidation (#391: calendar slot picker + legacy form removal), Session Notifications (#389, #390), Instructor Dashboard (#387), Dashboard Stats/Calendar (#385), Calendar/Sessions Migration (#383), Student Dashboard Fix (#382), Post-Purchase Email (#360, #375-381), Clerk Fixes (#370-371), Checkout UX (#373-374)

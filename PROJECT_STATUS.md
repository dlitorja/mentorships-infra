...
Last Updated: June 9, 2026 (Docstring coverage: PRs #401, #403, #405 (hooks) merged. PR #407 (15 Convex files) merged. PRs #408-#414, #416, #418, #419, #421, #423 merged - ~95% coverage achieved. Target of >90% exceeded.)
...
### NEW: JSDoc Docstrings - Documentation Coverage Initiative (June 8, 2026)

**Goal**: Reach 80% docstring coverage across platform code by documenting Convex functions, API routes, Trigger tasks, and components.

**Completed**:

**PR #401** (Merged June 8, 2026):
- ✅ Added JSDoc docstrings to all 18 functions in `apps/platform/lib/queries/convex/use-instructors.ts`
- ✅ Each docstring includes @description, @param, and @returns where applicable

**PR #403** (Merged June 8, 2026):
- ✅ Added JSDoc docstrings to all 22 functions in `apps/platform/lib/queries/convex/use-workspaces.ts`
- ✅ Added JSDoc docstrings to all 9 functions in `apps/platform/lib/queries/convex/use-sessions.ts`
...
- ✅ Phase 2: Documented 46 hook functions across use-instructors, use-workspaces, use-sessions
- ✅ Achieved 97.96% coverage threshold

**PR #407** (Open, June 8, 2026):
- ✅ Documented 15 Convex files with JSDoc comments (260 lines added)
- Files: admin.ts, contacts.ts, crons.ts, discordActionQueue.ts, instructorUploads.ts, legacyMappings.ts, migrationQueries.ts, monthlyStorageCosts.ts, notifications.ts, seed.ts, studentInvitations.ts, studentOnboarding.ts, studentSessionCounts.ts, userIdentities.ts, videoEditorAssignments.ts
- Greptile review: 5/5 confidence, no issues
- Note: instructors.ts excluded (had duplicate JSDoc issues)
...
**Goal**: Reach 80% docstring coverage on apps/platform API routes, then push to >90% across all code categories.

**Completed (PRs #408-#414)**:

**PR #408** (Merged June 8, 2026):
...
**PR #414** (Merged June 9, 2026):
- ✅ Documented `processStripeCheckout` in payments.ts - Stripe checkout fulfillment
- ✅ Documented `processStripeRefund` in payments.ts - Stripe refund processing
- ✅ Documented `processPayPalCheckout` in payments.ts - PayPal checkout fulfillment
- ✅ Documented `processPayPalRefund` in payments.ts - PayPal refund processing
- ✅ Documented `onboardingFlow` in onboarding.ts - Post-purchase onboarding
- ✅ Documented `syncInstructorInventoryToConvex` in inventory-sync.ts - Inventory sync

**PR #416** (Merged June 9, 2026):
- ✅ Documented `payments.ts` - 4 payment functions (processStripeCheckout, processStripeRefund, processPayPalCheckout, processPayPalRefund)
- ✅ Documented `sync.ts` - 8 sync handlers (syncPaymentCreated/Updated, syncOrderCreated/Updated, syncSessionPackCreated/Updated, syncSeatReservationCreated/Updated)
- ✅ Documented `discord.ts` - processDiscordActionQueue (cron job)
- ✅ Documented `booking-emails.ts` - 4 email handlers (handleSessionBookingEmails, handleSessionReminderEmails, handleSessionCancellationEmails, scheduleSessionReminders)
- ✅ Documented `onboarding.ts` - onboardingFlow (post-purchase onboarding)
- ✅ Documented `inventory-sync.ts` - syncInstructorInventoryToConvex
- ✅ Documented `clerk-user-linking.ts` - linkClerkUserToInstructor
- ✅ Documented `clerk-user-deleted.ts` - unlinkClerkUserFromInstructor
- Note: Removed unused getIdempotencyKey helper from sync.ts

**PR #418** (Merged June 9, 2026):
- ✅ Documented `send.ts` - 3 functions (sendEmail, sendTemplateEmail, formatSessionDateTime)
...
**PR #421** (Merged June 9, 2026):
- ✅ Documented calendar: BookWithGoogle, CalendarSlotPicker, BookSessionForm
- ✅ Documented instructor (12): BookSessionDialog, InstructorBookingsList, UpcomingSessionCard, PastSessionCard, RescheduleSessionDialog, CancelSessionDialog, SessionNotesDialog, SessionActions, StudentDetail, EmailPreviewTab, SchedulingSettingsForm, InstructorAvailabilityPreview
- ✅ Documented checkout: AvailabilityPreview
- ✅ Documented workspace (4): WorkspaceChat, WorkspaceNotes, WorkspaceImages, RetentionWarningBanner
- ✅ Documented dashboard: StudentOnboardingForm

**PR #423** (Merged June 9, 2026):
- ✅ Documented navigation (4): Header, Footer, ProtectedLayout, HeaderErrorBoundary
- ✅ Documented admin (3): AdminImageUpload, InstructorImageUpload, ImageUploadField
- ✅ Documented settings (2): GoogleCalendarCard, TimeZoneSelector
- ✅ Documented instructors (5): InstructorCarousel, InstructorNavigation, InstructorNavigationWrapper, PortfolioGallery, PortfolioLightbox
- ✅ Documented testimonials (1): TestimonialsCarousel

**Files Documented** (30 API handlers + 27 Inngest functions + 8 email template functions + 36 platform components across both apps):
...
- Platform Inngest: 6 payment/onboarding/inventory functions
- Web Inngest: 21 sync/email/discord/onboarding functions
- Email templates: booking confirmations, instructor notifications, series summaries, session cancellations/reschedules
- Platform components: calendar, instructor, checkout, workspace, dashboard, navigation, admin, settings, instructors, testimonials (36 functions)

**Coverage**: ~164 documented items across API routes + Inngest tasks + email templates + platform components (~95% overall)

**Status**: ✅ COMPLETED - PRs #408-#414, #416, #418, #419, #421, #423 merged (June 9, 2026). ~95% coverage achieved. Target of >90% exceeded.
...
**Last Updated**: June 9, 2026 (Docstring coverage: PRs #401, #403, #405, #407, #408-#414, #416, #418, #419, #421, #423 merged. ~95% coverage. Target of >90% exceeded.)
...
| Feature | Status | Notes |
|--------|--------|-------|
| **Docstring Coverage** | 🚧 IN PROGRESS | 4/9 use-* hooks documented (use-instructors, use-workspaces, use-sessions, session-emails). 5 remaining. |
...
### In Progress (June 8, 2026)
4. **Docstring Coverage**
   - ✅ PR #401: use-instructors.ts (18 functions)
   - ✅ PR #403: use-workspaces.ts (22 functions) + use-sessions.ts (9 functions)
   - ❌ Remaining: use-instructor-dashboard, use-mutations, use-users, use-session-packs, use-products, use-waitlist
